...
class Common_Icon extends Component{
  render(){
    const iconsList={
      marginRight: '16px',
      fontSize: '30px'
    ...
    return (
     <div>
       <div>1.基本用法</div>
       <div>使用 <Icon /> 标签声明组件，指定图标对应的 type 属性。可以通过 theme 属性来设置不同的主题风格的图标，也可以通过设置 spin 属性来实现动画旋转效果。</div>
       <div style={iconsList}>
         <Icon type="home" />
       ...
       <br/>
       <div>2.多色图标</div>
       <div>可以通过设置 theme 属性为 twoTone 来渲染双色图标，并且可以设置主题色</div>
       <div style={iconsList}>
       ...
       <br/>
       <div>3.自定义图标</div>
       <div>利用 Icon 组件封装一个可复用的自定义图标。可以通过 component 属性传入一个组件来渲染最终的图标，以满足特定的需求。s</div>
       <div style={iconsList}>
       ...
       <br/>
       <br/>
       <div>4.使用 iconfont.cn</div>
       <div>对于使用 iconfont.cn 的用户，通过设置 createFromIconfontCN 方法参数对象中的 scriptUrl 字段， 即可轻松地使用已有项目中的图标</div>
       <div style={iconsList}>
